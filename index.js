...
function setProgressCircle(percent) {
    ...
    const animate = () => {
        ...
        // Color logic
        if (current < 71) {
            progressCircle.style.background = '#e41e31'; // red
            percentageText.style.color = '#fff';
        } else {
            progressCircle.style.background = '#1ec41e'; // green
            percentageText.style.color = '#fff';
        }
        if (current !== end) {
            current += step;
...
function updateBars() {
    const bars = ['salesBar', 'stockBar', 'ordersBar', 'growthBar'];
    bars.forEach(barId => {
        ...
        const oldValue = parseInt(barValue.textContent);
        ...
        barTrend.className = 'bar-trend ' + (trend >= 0 ? 'positive' : 'negative');
        barTrend.innerHTML = `
        ...
        if (value < 40) {
            bar.style.background = 'linear-gradient(to top, var(--primary-color), #ff6b6b)';
        } else {
            bar.style.background = 'linear-gradient(to top, var(--secondary-color), #34eb7a)';
        }
    });
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    function nextAction() {
        if (idx < actions.length) {
            ...
            // Keep pressing randomly every 1s and update bars
            setInterval(() => {
                let delta = Math.random() > 0.5 ? 10 : -10;
                changePercentage(delta);
                updateBars(); // Update the bar charts
            }, 1000);
        }
    }
...
const dashboardData = {
    ...
    topSelling: [
        { name: "Premium Sweater", quantity: 171, unit: "pcs", trend: "+12%" },
        { name: "Designer Shirt", quantity: 45, unit: "sets", trend: "+8%" }
    ]
};
